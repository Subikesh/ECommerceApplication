...
class HomeViewModel(context: Application) : AndroidViewModel(context) {
    ...
    fun loadCategoryDatabase(categoryPair: MutablePair<Category, List<Product>?>) {
        viewModelScope.launch {
            categoryDatabase.insertCategory(categoryPair)
            Log.d("Database", "Category added: ${categoryPair.first.title_slug}")
            Log.d("Database", "Products added: ${categoryPair.second}")
        }
    }
}